...
export class QueryBuilder {
  ...
  findOne(tableName: string, options: QueryOptions = {}): any | null {
    ...
    return this.connection.prepare(query).get(...params) || null;
  }
  ...
  count(tableName: string, options: { where?: Record<string, any> } = {}): number {
    ...
    const result = this.connection.prepare(query).get(...params) as { count: number };
    return result.count;
  }
  ...
  aggregate(
    ...
    const result = this.connection.prepare(query).get(...params) as { result: number };
